...
class TerrainExporter:
    ...
    def create_gltf(self, heightmap, width, height, texture=None, sectors_x=1, sectors_y=1, water_texture=None):
        ...
        # ── Decimate + vectorise the display mesh ────────────────────────────
        # A full-res heightfield is ~W*H verts (e.g. 736² ≈ 542k verts / 1.08M
        # tris) — far more than a level-editor viewport needs, and the old pure-
        # Python double loop was slow to build too. Cap each axis at MAX_DIM
        # samples (keeping the last row/col so sector edges still meet) and build
        # the whole grid with NumPy. Terrain *editing* uses its own mesh, so this
        # only affects the display poly count.
        MAX_DIM = 256
        stride = max(1, int(np.ceil(max(width, height) / MAX_DIM)))
        ...
        denom_x = (width - 1) if width > 1 else 1
        denom_y = (height - 1) if height > 1 else 1
        nx = xs.astype(np.float64) / denom_x                 # (gw,)
        ny = ys.astype(np.float64) / denom_y                 # (gh,)
        NX, NY = np.meshgrid(nx, ny)                         # (gh, gw)
        PX = NX * real_width - sector_0_world_x
        PZ = NY * real_height - sector_0_world_z
        PY = heightmap[np.ix_(ys, xs)].astype(np.float64) * scale_z   # (gh, gw)
        # Interleave px,py,pz per vertex (row-major: index = gy*gw + gx).
        ...
        gy, gx = np.meshgrid(np.arange(gh - 1), np.arange(gw - 1), indexing='ij')
        i0 = (gy * gw + gx).ravel()
        i1 = i0 + 1
        i2 = i0 + gw
        i3 = i2 + 1
        tris = np.empty((i0.size, 6), dtype=np.int64)
        tris[:, 0] = i0; tris[:, 1] = i2; tris[:, 2] = i1
        tris[:, 3] = i1; tris[:, 4] = i2; tris[:, 5] = i3
