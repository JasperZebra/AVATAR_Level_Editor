...
class MapCanvas(QOpenGLWidget):
    ...
    def _get_map_center_gl(self):
        """Return the GL-space (x, y, z) centre of the loaded level for the sun orbit.

        Y is always 0 (ground plane) — the elevation angle lifts the sphere above
        the terrain naturally, so using average terrain height as the pivot would
        push the orbit off-center vertically.

        Priority:
          1. terrain_renderer extents (terrain_offset + terrain_world_w/h / 2)
          2. Terrain model AABB + world offset
          3. Entity XZ average
          4. Hard-coded Avatar default (512, 0, -512)
        """
        tr = getattr(self, 'terrain_renderer', None)
        tx = float(getattr(self, 'terrain_world_offset_x',
                           getattr(tr, 'terrain_offset_x', 0.0) if tr else 0.0))
        ty = float(getattr(self, 'terrain_world_offset_y',
                           getattr(tr, 'terrain_offset_y', 0.0) if tr else 0.0))

        # Best source: terrain_renderer knows exact world extents
        if tr is not None:
            tw = float(getattr(tr, 'terrain_world_w', 0.0))
            th = float(getattr(tr, 'terrain_world_h', 0.0))
            if tw > 0 and th > 0:
                cx = tx + tw * 0.5
                cz = -(ty + th * 0.5)   # world Y → GL -Z
                return cx, 0.0, cz

        # Second: terrain model GLTF bounds + offset
        model = self.terrain_model
        if model is None and getattr(self, 'terrain_models', []):
            model = self.terrain_models[0][0]

        if model is not None and model.bounds_min is not None and model.bounds_max is not None:
            bmin, bmax = model.bounds_min, model.bounds_max
            cx = (bmin[0] + bmax[0]) * 0.5 + tx
            cz = (bmin[2] + bmax[2]) * 0.5 - ty
            return cx, 0.0, cz

        # Third: entity XZ average (world → GL: x→x, y→-z)
        entities = getattr(self, 'entities', [])
        if entities:
            try:
                xs = [e.x for e in entities]
                zs = [-e.y for e in entities]   # world Y → GL -Z
                n = len(xs)
                return sum(xs) / n, 0.0, sum(zs) / n
            except Exception:
                pass

        return 512.0, 0.0, -512.0   # Avatar map default

    def _render_light_source_sphere(self):
        """Draw a Blender-style sun indicator: a blue sphere plus parallel rays
        showing the direction light travels across the map.

        The sphere sits at the sun position (map_centre + light_dir * dist).
        Rays shoot FROM the sphere TOWARD the map, matching how Blender's Sun
        lamp is visualised — confirming this IS a directional (parallel-ray) light.
        """
        try:
            pos = self._key_light_pos()              # unit direction FROM which light comes
            cx, cy, cz = self._get_map_center_gl()
            dist = 1400.0

            # The GL light vector points toward the sun, so the sun is in the
            # opposite direction — place the sphere there and shoot rays back.
            sx = cx - pos[0] * dist
            sy = cy - pos[1] * dist
            sz = cz - pos[2] * dist

            # Ray direction: from sphere toward the lit side of the map
            rdx, rdy, rdz = pos[0], pos[1], pos[2]

            # Build two vectors perpendicular to the ray direction so we can
            # spread the parallel rays around the sphere like Blender does.
            import numpy as _np
            ray  = _np.array([rdx, rdy, rdz], dtype=np.float32)
            up   = _np.array([0.0, 1.0, 0.0], dtype=np.float32)
            if abs(_np.dot(ray, up)) > 0.99:        # ray is near-vertical — use a different ref
                up = _np.array([1.0, 0.0, 0.0], dtype=np.float32)
            perp1 = _np.cross(ray, up);  perp1 /= _np.linalg.norm(perp1)
            perp2 = _np.cross(ray, perp1); perp2 /= _np.linalg.norm(perp2)

            # Offset positions for the parallel rays (ring around sphere + centre)
            spread = 55.0       # radius of the ring of rays
            ray_len = 560.0     # how long each ray line is
            offsets = [(0.0, 0.0)]   # centre ray
            n_ring = 8
            for i in range(n_ring):
                a = 2 * math.pi * i / n_ring
                offsets.append((math.cos(a) * spread, math.sin(a) * spread))
            ...
            glBegin(GL_LINES)
            for u, v in offsets:
                ox = float(perp1[0] * u + perp2[0] * v)
                oy = float(perp1[1] * u + perp2[1] * v)
                oz = float(perp1[2] * u + perp2[2] * v)
                # Line starts at sphere surface offset, ends toward map
                x0, y0, z0 = sx + ox, sy + oy, sz + oz
                x1 = x0 + rdx * ray_len
                y1 = y0 + rdy * ray_len
                z1 = z0 + rdz * ray_len
                glColor4f(0.3, 0.6, 1.0, 0.85)
                glVertex3f(x0, y0, z0)
                glColor4f(0.3, 0.6, 1.0, 0.10)   # fade to transparent at tip
                glVertex3f(x1, y1, z1)
            glEnd()
            ...
            glColor4f(0.15, 0.45, 1.0, 0.90)
            quad = gluNewQuadric()
            gluSphere(quad, 44.0, 24, 24)
            gluDeleteQuadric(quad)

            glColor4f(0.7, 0.85, 1.0, 0.70)
            quad2 = gluNewQuadric()
            gluSphere(quad2, 24.0, 16, 16)
            ...
            print(f"Light sphere render error: {e}")
    ...
    def _render_entities_3d(self, entities_sorted=None):
        ...
        if show_cubes:
            ...
            # Sort by color tuple so glColor3f only fires on actual color changes
            # (reduces GL state changes from N to ~number-of-unique-colors)
    ...
    def _on_glow_tick(self):
        """Drive the selection glow pulse repaint in 3D mode."""
        if self.mode == MODE_3D and self.selected_entity is not None:
            self.update()
    ...
    def _render_3d_opengl(self):
        ...
        try:
            ...
            # Draw entities (models always render, cubes conditional)
            if self.show_entities:
                ...
                self._render_entities_3d(visible)
                ...
                if _total > 400:
                    print(
                        f"⚠️ SPIKE {_total:.0f}ms: "
                        f"cull={(_t1-_t0)*1000:.1f} "
                        f"entities={(_t3-_t2)*1000:.1f} "
                        f"prims={(_t4-_t3)*1000:.1f} "
                        f"triggers={(_t5-_t4)*1000:.1f} "
                        f"shape={(_t6-_t5)*1000:.1f}"
                    )
            ...
            # Sun position indicator sphere
            self._render_light_source_sphere()
    ...
    def _get_map_filtered_entities(self):
            ...
            for _e in valid:
                ...
                if _mf:
                    ...
                    if _m and _m.bounds_min is not None and _m.bounds_max is not None:
                        ...
                        _diag   = float(np.linalg.norm(np.subtract(_m.bounds_max, _m.bounds_min)))
                        _r      = _scale * _diag * 0.5
                        _off    = (_m.bounds_min[1] + _m.bounds_max[1]) * 0.5 * _scale
                    else:
                        _r   = 4.0   # model not yet loaded — conservative sphere until bounds known
                        _off = 2.0   # assume centre is ~2 units above origin while loading
                _rad.append(_r)
                _yoff.append(_off)
        ...
        # Cache main-level entities (mapsdata, omnis, managers, sectorsdep, etc.) — always rendered, never culled
        self._never_cull_entities_3d = [e for e in valid if getattr(e, 'source_file', '') != 'worldsectors']
    ...
    def _get_interior_exempt_entities(self):
        ...
        # Pass 2 — for each anchor, find all entities whose world AABB overlaps it
        exempt_mask = inside.copy()
        for ai in anchor_idx:
            a_min = self._ic_w_min[ai]
            a_max = self._ic_w_max[ai]
            overlaps = (np.all(self._ic_w_max >= a_min, axis=1) &
                        np.all(self._ic_w_min <= a_max, axis=1))
            exempt_mask |= overlaps

        return [self._ic_entities[i] for i in np.where(exempt_mask)[0]]
    ...
    def _get_visible_entities(self):
        ...
        if self.mode == MODE_3D:
            ...
            if _n > 50000:
                FAR = 500.0   # FC2 full world: ~2K visible
            elif _n > 15000:
                FAR = 900.0   # FC2 per-cell: ~2-3K visible
            else:
                FAR = 1500.0  # Avatar / small levels
            NEAR = 0.1
            VFOV_DEG = 50.0
            FRUSTUM_PADDING = 1.4
            ...
            # Interior exemption: camera inside a building → keep that building + its contents rendered
            ...
            # Always include main-level entities (mapsdata/omnis/etc.) — append any frustum- or budget-culled ones
            ...
            if self._cull_log_frame >= 300:  # every ~5s at 60fps
                ...
                n_in_view = int(np.sum(frustum_mask))
                print(
                    f"⚡ CULL: {n_in_view}/{n_total} in frustum | "
                    f"{n_total - n_in_view} culled"
                )
