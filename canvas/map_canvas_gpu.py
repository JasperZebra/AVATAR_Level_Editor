...
class MapCanvas(QOpenGLWidget):
    ...
    def _apply_day_night(self):
        ...
        # Warmth echoed here too — the cool blue eases off as the sun gets low, so
        # the fill turns warmer through morning/afternoon and warmest at dawn/dusk.
    ...
    def _render_volumetric_rays(self, mvp, day, horizon, sd, cam_pos, vw, vh, default_fbo):
        ...
        # Modest base: the shafts stay always-on and visible from any angle, but no
        # longer flood the whole frame with glow (which was washing everything out /
        # over-brightening shaded areas). Stronger at a low sun for dawn/dusk drama.
        intensity = day * (0.55 + 0.55 * horizon)
    ...
    def setup_3d_models_for_level(self, worlds_path, resource_folder=None):
        ...
        if local_entitylib_path and os.path.exists(local_entitylib_path):
            ...
            try:
                ...
                self.model_loader.entity_patterns = load_entity_patterns(local_entitylib_path)
