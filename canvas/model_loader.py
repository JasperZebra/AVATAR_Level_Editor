...
# points, tag points, occlusion boxes and AI Duty/Social markers. Same invariant holds
# for FC2 worldsectors and Avatar mapsdata: 0 entities have a <resource> mesh without a
# graphic component, so a real mesh is never wrongly hidden. The editor mirrors this:
# an entity whose Components container lacks BOTH graphic components gets neither a 3D
# model nor a marker cube — it survives only as data (still selectable/editable via the
# object list / 2D map). Entities with no xml/Components to judge (e.g. editor-created
# objects) default to renderable so they are never suppressed.
...
def entity_has_graphic_component(entity):
    ...
    Cached on entity._has_graphic_component. Never raises — any parsing problem
    defaults to True (render). Defaults to True when there is no xml_element or no
    Components container, so editor-created entities that don't follow the full
    schema are never suppressed."""
...
class ModelLoader:
    ...
    def assign_models_to_entities(self, entities, progress_dialog=None, parent=None, game_mode="avatar"):
        ...
        for idx, entity in enumerate(entities):
            ...
            # NOTE: kit_model_files must stay a LIST ([], never None) — several call
            # sites iterate it via getattr(e,'kit_model_files',[]) with no None guard.
            if not entity_has_graphic_component(entity):
                entity.model_file = None
                entity.kit_model_files = []
