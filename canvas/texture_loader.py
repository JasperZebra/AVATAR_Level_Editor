...
Returns a rich XBMMaterialData so gltf_exporter can build a proper PBR
material with diffuse + normal + specular + emission textures, base
color tint, emission color, alpha mode, and double-sided flag — not just
a single diffuse like the old version.
...
class XBMMaterialData:
    ...
    def __init__(self):
        ...
        # ── Derived convenience fields (pre-extracted so gltf_exporter
        # doesn't need to crawl `properties` again) ────────────────────
