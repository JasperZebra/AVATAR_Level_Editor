...
class LevelSelectorDialog(QDialog):
    ...
    def on_change_resource_folder(self):
        ...
        try:
            ...
            # Step 4: Ask user about conversion if XBG files found
            should_convert = False
            if len(xbg_files) > 0:
                reply = QMessageBox.question(
                    self,
                    "Convert XBG Models?",
                    f"Found {len(xbg_files)} XBG model files.\n\n"
                    f"These need to be converted to GLTF format for 3D rendering.\n"
                    f"Conversion may take several minutes depending on model count.\n\n"
                    f"Convert now?\n\n"
                    f"(You can also convert later - models will auto-convert on first use)",
                    QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No
                )
                should_convert = (reply == QMessageBox.StandardButton.Yes)
            
            # Step 5: Batch convert XBG files if requested
            converted_count = 0
            if should_convert:
                progress_dialog.set_status("Converting XBG to GLTF...")
                progress_dialog.append_log("Starting batch conversion...")
                progress_dialog.append_log("This may take several minutes...")
                QApplication.processEvents()
                
                # Use ModelLoader's batch_convert_xbg_models method
                if hasattr(model_loader, 'batch_convert_xbg_models'):
                    converted_count = model_loader.batch_convert_xbg_models(progress_dialog)
                    progress_dialog.append_log(f"✓ Converted {converted_count} models")
                else:
                    progress_dialog.append_log("⚠ Batch conversion not available")
                    progress_dialog.append_log("Models will auto-convert on first use")
            ...
            # Step 6: Index all models (GLTF and converted)
            progress_dialog.set_status("Indexing GLTF models...")
            progress_dialog.append_log("Scanning for GLTF files (may take a moment on large folders)...")
            QApplication.processEvents()
            if scan_cancelled[0]:
                progress_dialog.mark_complete()
                progress_dialog.close()
            ...
            if success:
                ...
                progress_dialog.append_log(f"✓ Indexed {model_count} GLTF models")
                ...
                if converted_count > 0:
                    QMessageBox.information(
                        ...
                        f"✓ Converted {converted_count} XBG models to GLTF\n"
                        f"✓ Indexed {model_count} total GLTF models\n\n"
                        f"Models will load when you select a level."
                    )
                else:
                    remaining_xbg = len(xbg_files)
                    msg = f"Resource folder has been set up successfully!\n\n"
                    msg += f"✓ Indexed {model_count} GLTF models\n"
                    if remaining_xbg > 0:
                        msg += f"\n{remaining_xbg} XBG files will auto-convert on first use."
                    QMessageBox.information(
                        self,
                        "Resource Folder Updated",
                        msg
