...
`ObjectLibraryWidget` — a **searchable grid of model-thumbnail buttons** (`QToolButton`, 3 cols, `ToolButtonTextUnderIcon`): a filter box + a scroll grid of `get_library().all_names()`. Clicking a button → `place_archetype` (view-center drop; drag with the gizmo to position). **Thumbnails are rendered model previews:** `render_archetype_thumb` resolves the archetype to a model via `_resolve_model_for_archetype` (a throwaway proxy entity carrying the archetype's `<Entity>` XML → `model_loader.assign_models_to_entities` → `get_model_for_entity`) and calls the canvas's existing `render_model_thumbnail(model)` (offscreen FBO). Rendered **lazily** (one per `QTimer` tick — each loads a full model) and **disk-cached** to `_thumb_cache/objlib/<name>.png`. **Perf guard:** thumbnails auto-render only when the filtered list is ≤ `_THUMB_AUTO_LIMIT` (80) — loading hundreds of models at once would balloon memory; otherwise buttons are text-only until filtered. `refresh()` reloads the list (called on level load, after the entity library loads). `_object_library_widget` is stashed on the editor.
