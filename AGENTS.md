...
`ObjectLibraryWidget` — a **searchable grid of model-thumbnail buttons** (`QToolButton`, 3 cols, `ToolButtonTextUnderIcon`): a filter box + a scroll grid of `get_library().all_names()`. Clicking a button → `place_archetype` (view-center drop; drag with the gizmo to position).

**Thumbnails are real rendered models** (each archetype's own 3D mesh, like the AM3D library). `_resolve_model_for_archetype` resolves the model two ways: **Path A (reliable, the fix)** — `CFileDescriptorComponent.text_fileName` (e.g. `graphics\…\valkyrie.xml`) → `model_loader._extract_gltf_path_from_resource` (finds the real `.xbg`) → `load_static_xbg`. **Path B (kit/character)** — proxy entity → `assign_models_to_entities` → `get_model_for_entity`. (Path A covers ~1,500/2,436 hellsgate archetypes; the earlier proxy-only code matched a *texture* `<resource>` instead of the model — the "Tex Direhorse" bug.) `render_archetype_thumb` renders via the **mini model previewer** (`ModelPreviewWidget`) — a dedicated offscreen instance (`_get_thumb_previewer`, `auto_rotate` off, timer stopped, fixed 3/4 angle): `set_model(model)` uploads the model's `texture_raw_data` into the previewer's own GL context (so it textures correctly — the previewer draws from raw mesh arrays, context-independent), then `grabFramebuffer()` → `QImage`, scaled to the button size. `load_static_xbg`'s `_load_xbg_textures` stores `texture_raw_data` (`store_raw=True`), which is what makes this work. Rendered **lazily** (one per `QTimer` tick) and **disk-cached** to `_thumb_cache/objlib/<name>.png`. **Perf guard:** auto-render only when the filtered list is ≤ `_THUMB_AUTO_LIMIT` (80) — loading hundreds of models at once balloons memory; otherwise buttons are text-only until filtered. `refresh()` reloads the list (called on level load, after the entity library loads). `_object_library_widget` is stashed on the editor.
