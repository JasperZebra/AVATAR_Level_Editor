...
| `sequence_placement.py` | `tests/test_sequence_placement.py` | — | Viewport picking of cutscene handles: ray-vs-box math (incl. ray starting inside the marker), a 3D click picks the marker its ray crosses with the nearest winning, a 2D click tests the drawn 5 px square, picking offers only what the renderers DRAW (isolated node / rest marker only when the entity is missing, at the drawn half-extent), and the GL-ray→world plane mapping the 3D drag uses; GL ray + projector stubbed — excluded from `--cov` |
...
- Two handle kinds: `'key'` moves one Position keyframe (`SequenceLink.move_keyframe`), `'node'` moves the whole shot rigidly (`SequenceLink.on_entity_moved`, re-anchored every mouse-move since it is delta-based).
