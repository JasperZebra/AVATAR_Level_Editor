...
| `sequence_placement.py` | `tests/test_sequence_placement.py` | — | Viewport picking of cutscene handles: a 3D click picks the keyframe under the cursor (stubbed projector), the pick area tracks the marker's on-screen size (close-up face clickable, distant marker keeps the 8 px floor), picking offers only what the renderers DRAW (isolated node / rest marker only when the entity is missing), and the GL-ray→world plane mapping the 3D drag uses — excluded from `--cov` |
...
**Viewport picking of cutscene handles (`sequence_placement.py`, July 2026):** the diamonds/cubes drawn by `movie_renderer` are pickable in **both** views — `keyframe_at` projects through `_screen_projector(canvas)`, which is `canvas.world_to_screen` in 2D and a `gluProject` (FOV **50**, matching the render pass and `select_entity_3d`) in 3D, always returning LOGICAL pixels so `PICK_RADIUS_PX` means the same thing in each. Rules that matter:
- **The pick radius is adaptive, and it has to be.** The markers are drawn at a fixed WORLD size (ghost cube 1.5, camera key 1.2, diamond 0.8 — full extents), so a fixed 8 px radius made a marker you had flown up to — covering half the screen — respond only within 8 px of its centre, which reads as "the blocks aren't clickable". `_marker_radius_px` projects one world-radius offset per axis and takes the largest, clamped to `[PICK_RADIUS_PX, _MAX_PICK_PX]` (8..90 px); candidates are then ranked by `distance / radius`, not raw distance. A miss prints the nearest handle's pixel distance, which is the fastest way to tell "nothing drawn" from "radius too small".
- Clicking a handle also selects the node's **entity** (`_select_backing_entity`, mirroring the viewport-click path: `select_entity_with_children` → both gizmos → `entitySelected`), so the gizmo lands on the object and the ordinary move (which drags the path along) is available. Silent when the level has no such entity — there the marker is the only handle.
...
- **Not** gated on 3D Edit mode. It was briefly, to match the gizmo, and that alone made the handles feel dead in the default View mode — 2D never gated it either. Wired in `map_canvas_gpu.mousePressEvent` (after the gizmo hit-test, **before** `select_entity_3d` — the overlay is drawn depth-test-free on top), `mouseMoveEvent`, `mouseReleaseEvent`; the 2D hooks live in `canvas/input_handler.py`.
