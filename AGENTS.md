...
Worked example (Aug 2026 fixes): square water planes, world-anchored reflections, the black-model tint guard, and the slot-hardening sweep are all in shared code — both games got them at once. The former standing gap — `tools/create_sector.py` assuming the Avatar grid — was closed the same month (see "Sector tools are game-aware" below).
...
| `canvas/entity_renderer.py` | `tests/test_entity_label_2d_ints.py` | — | `_draw_entity_label_2d_optimized` must pass INTEGER coords to `fillRect`/`drawText`: `sx`/`sy` arrive as `world_to_screen` floats and sip rejects them outright, so selecting any entity in 2D raised `TypeError: ... unexpected type 'float'` every frame; stub painter enforces the int-only contract, module imported normally (relative imports rule out the load-by-path trick) — excluded from `--cov` |
...
| `canvas/water_plane_renderer.py` | `tests/test_water_full_squares.py` | — | Water is ALWAYS the sector's full square plane (Aug 2026 user decision): `_emit_cell` emits one 64×64 quad even for partially-submerged or fully-dry heightmaps (the old span clipping / SKIP are dead); loaded by **file path** — excluded from `--cov` |
| `canvas/model_shader.py` + `canvas/gpu_driven_renderer.py` | `tests/test_reflection_world_anchored.py` | — | Reflection cubemaps sampled by the WORLD normal `Nw` (reflections anchored to the model), never `reflect(Vw,…)` (camera-tracking); shader sources scanned as **text** — excluded from `--cov` |
| `canvas/gpu_driven_renderer.py` | `tests/test_effective_diffuse_tints.py` | — | `effective_diffuse_tints`: black tint+base with NO loaded reflection cubemap → white (diffuse texture shows, previewer parity); black WITH reflection kept; coloured/single-black untouched; module by **file path** — excluded from `--cov` |
| `tools/enable_all_sectors.py` + `tools/create_sector.py` + `simplified_map_editor.py` | `tests/test_tools_scripts_pyqt5.py` | — | In-process tools/ GUI scripts must import PyQt5 (PyQt6 into the PyQt5 process = hard crash) and the two menu handlers must be fully try/except-wrapped (PyQt5 aborts on slot exceptions); sources scanned as **text** — excluded from `--cov` |
...
**QPainter wants INTS in this module.** `sx`/`sy` come out of `world_to_screen` as floats and PyQt5's sip does **not** truncate — the int overloads of `fillRect(x, y, w, h, c)` and `drawText(x, y, text)` raise `TypeError: argument 1 has unexpected type 'float'`. `_draw_entity_label_2d_optimized` was the one place that forgot to `int()`, and since labels only draw for **selected** entities, selecting anything in 2D aborted the label pass and buried the console with that traceback every frame. Either cast, or pass a `QRectF`/`QPointF` (the float overloads) — the rest of the module casts.
...
**Selecting a node row gives it a GIZMO:** `_select_entity_for_movie_node` routes through `on_entity_selected(ent)` — the same handler a viewport click uses — instead of only setting `canvas.selected/selected_entity`. It used to do the latter, so a node picked in the Sequences tab was "selected" with **no handle to move it in either view** (3D reads `gizmo_3d`, which only `on_entity_selected` / a viewport click positions). Nodes whose EntityId this level does not have still just report that in the status bar.

**Viewport picking of cutscene handles (`sequence_placement.py`, July 2026):** the markers drawn by `movie_renderer` are pickable in **both** views, and each view uses the SAME test its other objects use. Rules that matter:
- **3D is a ray cast, like models / fallback boxes / marker cubes.** `_pick_ray` unprojects the cursor exactly the way `select_entity_3d` does (FOV **50**, matching the render pass), `_ray_box_t` is the slab test against each marker's own box, and the nearest hit along the ray wins — so overlapping markers resolve by depth, not by pixel distance. Marker half-extents come straight from what `movie_renderer` draws: diamond 0.4, camera key cube 0.6, rest marker 0.75 (`_HALF_*`); camera-ness is decided by `cs_camera_preview.is_camera`, the same call the renderer makes. **2D is the drawn square** (`_MARKER_2D_HALF_PX` = 5, what `_draw_diamond_2d` / the ghost square use), i.e. the same rule `get_entity_at_position` applies to the entity squares — 2D markers are a fixed PIXEL size at any zoom, so nothing about the pick should scale with distance there.
- A screen-space circle of `PICK_RADIUS_PX` is kept as a 3D **rescue only**: at range a marker is a pixel or two wide and the ray can miss what you can plainly see. Up close the ray has already decided. Do not let this grow into the primary test — a fixed pixel radius is exactly what made close-up markers feel unclickable (only their centre ±8 px responded while the block covered half the screen).
- A miss prints the nearest handle's pixel distance, which is the fastest way to tell "nothing was offered for picking" from "you missed it".
- Clicking a handle also selects the node's **entity** (`_select_backing_entity`, mirroring the viewport-click path: `select_entity_with_children` → both gizmos → `entitySelected`), so the gizmo lands on the object and the ordinary move (which drags the path along) is available. Silent when the level has no such entity — there the marker is the only handle. `_highlight_in_tree` then moves the Sequences tab's selection to that node **with signals blocked** (the caller already set `selected_movie_node_id`; re-entering `_on_sequence_selected` would only redo the work), so a viewport pick is visible in the tab.
- `_pick_targets` mirrors what is **drawn**: only the isolated node when `selected_movie_node_id` is set, and a node's rest marker only when its `entity_id` is missing from `canvas.entities` (otherwise the entity IS the handle). Picking something invisible is a bug.
- Two handle kinds: `'key'` moves **that one** Position keyframe and nothing else (`SequenceLink.move_keyframe`, an absolute write by index), `'node'` moves the whole shot rigidly (`SequenceLink.on_entity_moved`, delta-based so it is re-anchored every mouse-move). Both re-anchor `hit["world"]` each move — the drag plane is built through it.
- **Height needs SHIFT.** The horizontal-plane drag holds the key's Z by construction, so without a vertical mode a 3D key could only ever slide sideways. Shift swaps in `_ray_vertical_world`: the ray against the vertical plane through the anchor whose normal is the ray's own HORIZONTAL direction (so it always faces the camera and stays well-conditioned), with X/Y held. Returns None when the camera looks straight down — no horizontal component to build the plane from. 2D stays X/Y only; that is all a top-down view can express. The status bar shows the live value during the drag and the console prints the saved one.
- **The drag has to touch BOTH models**: the renderers read `main_window.movie_data`, the writes go through `SequenceLink`'s own ElementTree (`_set_key_in_memory` / `_shift_node_in_memory`). Without the in-memory half, the path only jumps to its new shape after `end_keyframe_drag` reloads the file.
- 3D drag = view ray × the **horizontal plane through the handle** (`_ray_plane_world`, the inverse of `world(x,y,z) → gl(x,z,-y)`), so a camera path slides at its own height instead of dropping to the terrain. Terrain snap applies only with `pending_sequence_snap` or (3D) `terrain_snap_enabled`.
- `SequenceLink.attach` wraps `canvas._auto_save_entity_changes` **once** (`_sequence_link_attached`), so the wrapper resolves `canvas.sequence_link` at call time and `attach()` rebinds it on every call. Closing over the link instead meant a second level's cinematic moves were written into the FIRST level's moviedata.xml. `_attach_sequence_link` clears both `self.sequence_link` and `canvas.sequence_link` before rebinding, and the no-moviedata branch of the level load clears them too.
- **Not** gated on 3D Edit mode. It was briefly, to match the gizmo, and that alone made the handles feel dead in the default View mode — 2D never gated it either.
- **Click order is the same in both views: gizmo (and 2D shape points) → cutscene handles → entities.** The handles must come AFTER the gizmo because keyframe 0 usually sits exactly on its entity and would otherwise swallow the gizmo's centre handle; they come BEFORE entity picking because the overlay is drawn depth-test-free on top. 3D lives in `map_canvas_gpu.mousePressEvent`/`mouseMoveEvent`/`mouseReleaseEvent`; 2D goes through `InputHandler._begin_sequence_pick`, which **reports** exceptions — it used to sit behind a bare `except: pass` in two places, so a broken pick was indistinguishable from "the markers aren't clickable".

**Sequences-tab right-click menu (`on_sequence_tree_context_menu`):** add/duplicate/rotate/export per sequence or per node, "Remove All Cameras (make scripted event)", and **"Remove Sequence '<name>'..."** (`_seq_remove_sequence` → `sx.remove_sequence(root, name)`, confirm dialog quotes the node + key count). Rules learned the hard way here:
- **`bundle.node_defs` is a filtered VIEW on a live tree.** `_sequence_live_bundle` builds it by *filtering* `<NodeData>`, so `node_defs.remove(elem)` changes a Python list and nothing else — deleting a camera left its `<Node Name="Camera..." EntityId=... Pos=...>` registry entry in the file with no keys and no sequence referencing it. `SequenceBundle.source_root` (set only for live bundles) lets `remove_node`/`strip_camera_nodes` call `sx.drop_node_defs(root, ids)`, which removes a registry entry **only when no sequence anywhere in the file still references that id** — a node shared by two sequences survives losing one. Empirically an orphan NodeDef is never legitimate: across all 260 shipped moviedata files (Avatar + FC2) all 1438 registry entries are referenced.
- **`_sequence_save` has to re-sync three things**, because these helpers edit the raw ElementTree: reload `movie_data` (the PARSED model the tree and both renderers draw from — otherwise a removed node keeps rendering until a level reload), re-attach `sequence_link` (its own parse of the same file; a later keyframe drag would save the pre-edit content back over the removal), and refresh the tab. It used to call `refresh_sequences_tree` behind a `hasattr` guard — **that method does not exist** (`update_sequences_tab` is the real one), so the guard silently skipped the refresh entirely. It also drops a `selected_movie_sequence` / `selected_movie_node_id` that the edit just deleted.
...
**Water is ALWAYS the full square sector plane (Aug 2026 user decision).** The July 2026 submerged-cell clipping (`_get_submerged_geometry`: heightmap < water height per cell, span emission, FC2 height-clip veto) carved the quads into ragged terrain-following shapes; the user rejected that look. `_emit_cell` now hardcodes `geom = 'FULL'` — one flat square quad per watered sector, no heightmap consultation. `_get_submerged_geometry` and the span branch are kept in the file but are DEAD in the render path; don't re-enable clipping without asking. Regression test: `tests/test_water_full_squares.py`.

**Model reflections are WORLD-ANCHORED (Aug 2026 user decision).** Both material shaders (`model_shader.py` legacy instanced + `gpu_driven_renderer.py` GDR) sample the reflection cubemap by the world-space NORMAL `Nw`, not `reflect(Vw, Nw)` — the view-dependent lookup made reflections slide with the camera, which the user reported as a bug. Don't reintroduce a view vector into the cubemap lookup. Regression test: `tests/test_reflection_world_anchored.py`.

**Black-model guard — `effective_diffuse_tints` (Aug 2026).** Glass/chrome materials author black `DiffuseColor1`/`DiffuseColorBase` and get their in-game colour from the reflection cubemap. When that cubemap fails to load (decode returns None), a black tint × diffuse texture = pure black model in the canvas while the mini previewer (raw textures, no tints) looks fine. `effective_diffuse_tints(tint, tint_base, has_reflection)` (module-level in `gpu_driven_renderer.py`, pure) substitutes white tints ONLY when both tints are near-black AND no reflection texture loaded. Applied in BOTH paths: `_build_material_table` (GDR) and `model_loader._draw_mesh_instanced`. The fixed-function fallback doesn't apply tints, so it needs no guard. Test: `tests/test_effective_diffuse_tints.py`.
...
### PyQt5 fix: tools/ GUI scripts exec'd IN-PROCESS must be PyQt5 (Enable All Sectors / Create New Sector crash, Aug 2026)

Clicking `Tools ▸ Enable All Sectors…` or `Tools ▸ Create New Sector…` killed the whole editor. The June 2026 migration deliberately skipped the gitignored `tools/` folder ("not part of the shipped app") — but `open_enable_all_sectors` / `open_create_sector` load `tools/enable_all_sectors.py` and `tools/create_sector.py` via `spec_from_file_location` **into the editor's own process**. With PyQt6 installed on the machine the import *succeeds*, Qt6 DLLs load next to the live Qt5 ones, and the process hard-crashes when the tool window is created (no Python traceback — it's a native Qt conflict). Both scripts are now ported to PyQt5 (imports + `app.exec_()`; every other API they use is identical), with a warning comment at the import site.

Hardening added at the same time: PyQt5 **aborts the app** (qFatal) on any unhandled Python exception inside a Qt slot — so both menu handlers now route through `_load_tools_script(name)` (missing/broken script → `QMessageBox.warning`, not a crash) and wrap their whole body in try/except. Frozen builds don't ship loose `tools/*.py`, so without this guard the two menu items would crash the release exe every time. **Rule: anything imported into the editor process must be PyQt5, and every Qt slot body must be exception-proof.** Regression test: `tests/test_tools_scripts_pyqt5.py`.

### Slot-hardening sweep (Aug 2026) — every menu/button/shortcut audited

A full audit of every `.connect()` target across the app (78 modules, AST-checked: zero connect-target typos, zero remaining PyQt6 imports) found ~16 handlers whose body could raise out of a Qt slot (= abort under PyQt5). All are now guarded with try/except + traceback + a `QMessageBox.warning` where user-initiated. Guarded in `simplified_map_editor.py`: `open_world_editor`, `open_water_editor`, `open_entity_library_browser`, `open_entity_editor`'s already-exists `set_entity` branch (2nd Ctrl+E), `_open_mp_spawn` (canvas right-click), `_on_save_finished`/`_on_save_failed` (queued save slots), `update_entity_tree` (reached from save/paste/filter — one malformed entity degrades to a console traceback), the terrain-paint `_save_tex_clicked`/`_refresh_tex_clicked` wrappers (the refresh imports PIL inside the slot), and the five `_seq_*` sequence context handlers (their `except KeyError` became `except Exception` and now covers the `_sequence_save` tail). Also: if `setup_complete_smart_system` fails at startup, no-op stubs are installed for the runtime-bound copy/paste methods (`select_all_entities` etc.) so the first right-click/Ctrl+C shows a warning instead of AttributeError-aborting. Guarded elsewhere: `world_editor._browse`+`load`, `object_library._on_click`+`_render_some` (thumbnail QTimer — failed archetypes are remembered and never retried), `sequence_import_dialog._start_placement`, `entity_export_import.browse_for_collection`, `all_in_one_copy_paste.copy_selected_entities`/`show_clipboard_info`/`paste_entities`, `terrain_editor_dialog._save_terrain`, and five `entity_editor.py` add/remove row handlers. NOT changed: the original `select_level` (unguarded but unreachable — `integrate_patch_manager` replaces it at startup with `new_select_level`, which has full try/except + a re-entry lock). **Rule for new UI: any function connected to a signal (or QTimer/QShortcut) must not let an exception escape.**
...
### App-wide dialog theming — ALL dialogs follow the Light/Dark preference (Aug 2026)

**Rule: every dialog/tool window must style itself via `theme_settings.apply_dialog_theme(self)` (called once, after its UI is built) — never hardcode a dark or light look.** The user's Canvas ▸ Light/Dark Mode preference (`force_dark_theme`, saved in `editor_config.json`) now drives the whole app, not just the main window.

The API (module-level in `theme_settings.py`):
- `apply_dialog_theme(widget, dark=None)` — resolves the theme (walks widget parents for a live `force_dark_theme`, else reads the saved config), applies the shared dialog stylesheet (palette matches the main window's `apply_theme()`: `#2b2b2b` dark / `#f0f0f0` light), tags the window (`_theme_dialog`), and calls the widget's optional `_retheme(dark)` hook.
- `_retheme(dark)` — put anything the shared stylesheet can't express here (per-item tree colors, syntax highlighting, custom header bars, semantic status labels). It re-runs on live toggles, so restyle/rebuild whatever carries theme colors. Keep it exception-safe.
- `theme_colors(dark)` — the raw palette dict for custom styling.
- `retheme_open_windows(dark)` — called by the main window's `toggle_theme`; re-themes every open tagged window live.

Converted (Aug 2026): `entity_library_browser` (per-theme item colors + theme-aware XML syntax highlighter — the model conversion to copy), `entity_editor` (accent buttons/groups via `_ACCENT` dicts, xml editor, checkbox styles), `world_editor` (its 15-line hardcoded dark sheet deleted; XBT thumbnail tiles retheme live without a tab rebuild so unapplied raw-XML edits survive a toggle), `water_editor_dialog`, `terrain_editor_dialog` (dirty label), `mp_spawn_creator` (spawn-ok green), `sequence_import_dialog` (both), `pak_ui`, `game_selector`, `set_patch_folder`'s LevelSelectorDialog (local theme machinery deleted in favor of the shared one), `entity_export_import` (both), the tools windows `create_sector`/`enable_all_sectors` (guarded `_apply_editor_theme` — sys.path-inserts the project root like their converter helpers; standalone use falls back to the system palette), and `EnhancedProgressDialog` — the startup "Initializing…" / level-load / save progress dialog (one class, six call sites; it shows BEFORE the main window exists, where `apply_dialog_theme` falls back to the saved config preference; its guarded so the boot dialog can never kill startup, and the console-style log box switches colors via `_retheme`).

Conventions: hardcoded light-on-dark text needs a darker-ink light variant (`#9CDCFE`→`#0451A5`, `#4EC9B0`→`#0B7261`, `#d4d4d4`→`#1e1e1e`); semantic status colors keep their meaning in both themes (ok green `#2e7d32`, warn orange `#b45f06`); mid-grey `#888` dim labels are fine on both; compact toolbar buttons use size-only styles (`_BTN_COMPACT`) so colors inherit. Regression test: `tests/test_dialog_theme.py`.

### Entity Library Browser is an EDITOR now — dark theme + BinHex-synced edits (Aug 2026)

`entity_library_browser.py` (Tools ▸ Entity Library Browser; the user calls it the "FCB editor") was a read-only viewer that hardcoded light text colors (#d4d4d4/#9CDCFE) with NO stylesheet — on the system light palette everything rendered white-on-white and only became readable when a row was selected. It now themes itself via `apply_dialog_theme` (see the app-wide theming section above; per-theme `_ITEM_COLORS` + theme-aware `_XmlHighlighter`) and is an editor. The Simple tree has alternating row colors OFF (user request: the stripes read as dark/light/dark bands that made the text hard to read — every row keeps the single theme background; don't turn `setAlternatingRowColors` back on here) and 26px min-height rows with a sized inline editor (default-height rows vertically CLIPPED the QLineEdit's text when a value cell was clicked — keep the size-only per-widget stylesheet on `_simple_tree`):

- **Simple tab**: clicking a field's Value cell opens an inline editor (`NoEditTriggers` + programmatic `editItem(item, 1)` so the name column can never be edited). A commit goes through `_apply_field_edit`, which updates the cosmetic `value-*` attribute AND regenerates the **authoritative BinHex text** — the native FCB converter reads the BinHex ("value-* are cosmetic"), so skipping that sync means the edit silently never reaches the regenerated .fcb. Bad input reverts the cell with a warning. Structural fields (child elements, `__rawhex` RML fields) are read-only — for `__rawhex` fields the text is IGNORED by the converter, and for expansions editing text would corrupt the re-encode.
- **XML tab**: editable (like the entity editor's XML tab) with an **Apply Changes** button: parses the text, runs `_resync_binhex` (every simple field's hex regenerated from its value-* attr — the friendly value WINS over hand-edited hex on such fields), and swaps the new element into the library in place. Truncated displays (> `_XML_DISPLAY_LIMIT`) stay read-only — applying a cut-off dump would destroy the prototype.
- **Save**: top-bar button, threaded `_SaveWorker` (libraries are tens of MB — GUI-thread serialisation would hang), `.bak` of the original on first save, dirty `*` in the title, Save/Discard/Cancel prompt on close (close-path save is synchronous — no event loop left for a worker). NOTE: `ET.parse` drops the leading `fcb_meta` comment, so a saved file fresh-encodes on XML→FCB — that is the converter's documented edit-safe path. Reconversion stays with Tools ▸ Convert Entity Library XML to FCB.

**Encodings retail-verified** (FC2 world1 entitylibrary, applies to Avatar too — same converter): `value-ComputeHash32` stores **CRC-32 of the case-sensitive string** (`zlib.crc32`, NOT the djb2 variant entity_editor.py uses for its own compute_hash32!); `Hash32`/`UInt32`/`Enum` = LE uint32 of the shown integer; `String` = null-terminated ASCII; `Float32`/`Vector2/3/4` = LE floats; `Boolean` = one byte 01/00; `Id64` = LE uint64. Value-* inventory observed: String, Float32, Int32, Boolean, Hash32, ComputeHash32, Vector3, Enum, Vector4, UInt32, Id64, Vector2. Regression test: `tests/test_library_browser_binhex.py`.

### Sector tools are game-aware + use the native FCB converter (Aug 2026)

`tools/create_sector.py` and `tools/enable_all_sectors.py` were Avatar-only in two ways, both fixed and ground-truthed against retail FC2 world1 (`w1_c_3`):

**Grid.** `CreateSectorWindow(..., game_mode=...)` (passed by `open_create_sector`) resolves `GRID_STRIDE = {'avatar': 16, 'farcry2': 80}`. All X/Y math is `sector_xy(id, stride)` — retail `worldsector2592` has `X=32, Y=32` (= 2592 % 80, 2592 // 80) with the SAME field hashes as Avatar (`C1CB6D9A`/`2ABD43F2`/`B7B2364B`/`C0B506DD`, MissionLayer `494C09F2`), so one template serves both games. FC2's per-world `world1.sectorsdep.xml` has **6400** CWorldSector entries (80×80) with Avatar-identical flag hashes (`HasDescriptor 93880D75`, `HasMainSectorData 346F3F63`, `HasLandmarkNear 97C2E974`, `HasLandmarkFar F19B593D`), so the name-matched sectorsdep patching needed no changes. An FC2 cell folder owns ONE 16×16 block of the global grid: `cell_id_block(ws_dir, stride)` derives it from any sector/landmark/desc file present (e.g. w1_c_3 → IDs 2592–3807 in 16-wide rows); bulk create iterates that block (NEVER `range(256)` or the whole grid — that would spray other cells' sectors into this folder), aborts with a message when the block can't be derived (empty folder), and single-create refuses IDs outside the block. Avatar keeps 0–255. Retail sanity check: w1_c_3 scan finds 26 missing worldsector files = 256 − ~230 retail count. Regression test: `tests/test_create_sector_fc2.py`.

**Conversion.** Both tools shelled out to `tools/FCBConverter.exe`, which the project no longer ships (native converter, June 2026) — so their FCB steps silently skipped for BOTH games. They now route through `file_converter._native_fcbconvert` via `create_sector.run_fcb_conversion(args)` (exe-style args; keeps `-fc2` per the always-present rule; **splits comma filters** — the native engine's fnmatch takes one glob, unlike the exe), with the exe as a standalone-use fallback. `enable_all_sectors` imports these helpers from its sibling (`_converter()` puts tools/ on sys.path). Bulk create converts ONLY the newly written XMLs (explicit file list), not a `worldsector*` wildcard — a wildcard would reconvert every `.converted.xml` the editor left after loading and litter `_new.fcb` orphans.
...
### PyQt5 fix: `Qt.Checked.value` is PyQt6 syntax — crashed the day/night checkboxes (Aug 2026)

Toggling "God rays" (and "day/night enable" — identical bug) aborted the editor: their `stateChanged` slots compared `state == _Qt.Checked.value`. PyQt5 sip enums have **no `.value`** (that's PyQt6's scoped-enum accessor), so the slot raised AttributeError → qFatal. In PyQt5 `stateChanged` passes a plain int and `Qt.Checked` compares as one — the correct form is simply `state == Qt.Checked`. Repo-wide sweep found only these two sites. Lesson: the June migration de-scoped `Qt.X.Y` → `Qt.Y` but missed `.value` accessors, which COMPILE fine and only crash when the widget fires. Regression test: `tests/test_no_pyqt6_enum_leftovers.py` (text-scans every app source for `Qt.<Enum>.value` patterns).
...
### Multiplayer stats — `tools/DevAccess/MP_STATS.md` (Aug 2026)

Where the MP stats live, for the "push end-of-match stats to our server" work.
Companions: `mp_stats.json` (Avatar, 105 stats) and `mp_stats_fc2.json` (FC2,
179), both regenerated by `extract_mp_stats.py <gamemodesconfig.xml> [out.json]`.

The four findings worth carrying without re-deriving:

1. **Stats are fully tracked and go nowhere.** `CGameStatsService` computes 84 MP
   stats, live-replicates 37, and fires `CGREventEndGameStatsReceived` at match
   end. Avatar's `Dunia.dll` has **no** GameSpy, account service, leaderboard or
...
3. **`ATGE/patch/…/gamemodesconfig.xml` is Dunia BINARY xml** (`00 00 FF 83`) and
   ...
   that file**. Decode with `tools/convert_avatar_xml.py`.
4. **Avatar has no per-weapon stats.** FC2's 79 extra stats are all
   `Kills_*`/`Headshots_*`/`Executions_*` per weapon. The list is data-driven XML
   so declaring them in Avatar is easy; whether anything still *increments* them
   is untested.
