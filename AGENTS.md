...
### `.ai.rml` prefix REGENERATION (in progress June 2026 — toward add/remove editing)
Goal: rebuild the opaque prefix purely from the node tree so structural edits (add/remove/rename nodes) repack byte-exact, not just value edits. Findings so far:
