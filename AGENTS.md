...
| `sequence_placement.py` | `tests/test_sequence_placement.py` | — | Viewport picking of cutscene handles: a 3D click picks the keyframe under the cursor (stubbed projector), picking offers only what the renderers DRAW (isolated node / rest marker only when the entity is missing), and the GL-ray→world plane mapping the 3D drag uses — excluded from `--cov` |
...
- Gated on Edit mode in 3D (`input_handler.edit_mode_3d`) exactly like the gizmo — in View mode `begin_keyframe_drag` returns False so the click falls through to entity selection. 2D keeps its own gating. Wired in `map_canvas_gpu.mousePressEvent` (after the gizmo hit-test, **before** `select_entity_3d` — the overlay is drawn depth-test-free on top), `mouseMoveEvent`, `mouseReleaseEvent`; the 2D hooks live in `canvas/input_handler.py`.
