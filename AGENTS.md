...
`entity_library_browser.py` (Tools ▸ Entity Library Browser; the user calls it the "FCB editor") was a read-only viewer that hardcoded light text colors (#d4d4d4/#9CDCFE) with NO stylesheet — on the system light palette everything rendered white-on-white and only became readable when a row was selected. It now sets a full dark stylesheet (`_DARK_STYLE`: trees, tabs, headers, line edits, scrollbars) and is an editor:
