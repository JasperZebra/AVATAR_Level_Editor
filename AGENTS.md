...
**Fix:** at the END of `paintGL`, request the next frame (`self.update()`) while the camera is actively moving — 3D `camera_3d.needs_update()` or 2D `camera_controller.needs_update()`. Because the swap that follows paintGL blocks on vsync, a repaint is always pending the instant it returns, so the **swap itself** paces presentation at the true refresh rate (60). The loop self-terminates when the movement keys are released, so idle cost is unchanged. **Movement speed is untouched** — the 3D camera is still stepped at fixed `0.1`/tick by the 16ms timer (the 3D `Camera3D.update_movement` is fixed-step, NOT dt-based; the 2D `camera_controller` already is dt-based). Relies on vsync being on (it is — the 1.5-vblank symptom proves it); with vsync off the loop would instead be capped by frame cost. **Not unit-tested** — pure GL/Qt paint-loop scheduling, GL-context + QApplication bound (the whole `map_canvas_gpu.py` can't be imported headlessly), consistent with the rest of the canvas render code; verified in-app by the FPS counter.
