...
# ============================================================================
# PyOpenGL PERFORMANCE FLAGS — MUST run before the FIRST `import OpenGL.*`.
# By default PyOpenGL wraps EVERY GL call with a glGetError() round-trip + array
# validation. A 3D frame issues tens of thousands of GL calls, so that per-call
# validation — not the GPU — dominates CPU time (it's the bulk of the ~390 ms
# "entities=" render spikes). Disabling it is the canonical PyOpenGL speedup
# (commonly 2-5x on call-bound rendering) with no visual change.
#
# Safety: with checking off, GL errors no longer raise Python exceptions, so the
# shader path adds a cheap ONCE-PER-FRAME glGetError() probe (model_loader) to
# still detect/fallback on real errors. Set OPENGL_DEBUG=1 to keep full per-call
# checking on while debugging (GL errors raise immediately again).
# ============================================================================
if not os.environ.get('OPENGL_DEBUG'):
    try:
        import OpenGL
